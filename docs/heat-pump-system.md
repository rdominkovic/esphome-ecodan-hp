...
| Apr 2 | ~09:00 | **Overnight report with true COP and heat loss** | Upgraded `scripts/overnight_report.py` (deployed to VM at `/opt/ecodan/scripts/`). Three additions: (1) **True COP** from FTC energy counter deltas (`daily_produced - daily_consumed` per cycle) instead of unreliable `estimated_cop` sensor. Counters update every ~2 min, sufficient for 25+ min cycles. Midnight counter reset cycles auto-skipped. (2) **Heat loss estimate** via energy balance: total heat delivered / hours / temp diff = loss coefficient. Measured 0.125 kW/°C — at OT 7 C house needs 2.0 kW, at OT 0 C needs 2.8 kW, at OT -5 C needs 3.5 kW. Key finding: above OT 10 C demand <1.6 kW is below heat pump minimum output → cycling unavoidable. (3) **OT-COP breakdown** groups cycles by 2-degree outdoor temp buckets with true COP. Run: `python3 /opt/ecodan/scripts/overnight_report.py [YYYY-MM-DD]`. |
